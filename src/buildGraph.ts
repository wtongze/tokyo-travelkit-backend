...
const calendarList = [
  ...
  {
    id: 'odpt.Calendar:SaturdayHoliday',
    key: 'saturdayHoliday',
  },
];

(async () => {
  for (const cName of calendarList) {
    const timetables = await TrainTimetable.findAll({
      where: {
        odptOperator: operatorList,
        odptCalendar: cName.id,
      },
    });
    ...
    for (const currTimetable of timetables) {
          ...
          if (previousTimetable) {
            ...
            if (lastItem.odptArrivalTime && firstItem.odptDepartureTime) {
              graph.addEdge(
                ...
                getTimeDiff(
                  lastItem.odptArrivalTime,
                  firstItem.odptDepartureTime
                )
              );
            }
          } else {
      ...
      for (const currStopItem of currTimetable.odptTrainTimetableObject) {
        ...
        if (index < maxIndex) {
          const nextStopItem =
            currTimetable.odptTrainTimetableObject[index + 1];
          if (
    ...
    fs.writeFileSync(getCachePath(`${cName.key}Graph.txt`), graph.toTXT());
    fs.writeFileSync(
      getCachePath(`${cName.key}TimeDict.json`),
    ...
    fs.writeFileSync(
      getCachePath(`${cName.key}TimetableStationMap.txt`),
      content
    );
    ...
    fs.writeFileSync(
      getCachePath(`${cName.key}HeuristicMap.txt`),
